...
完整复现顺序如下。第一条命令生成固定的100条 test 清单；第二条只读取 E 盘模型缓存完成推理；第三条创建 Excel 可直接打开的人工评测表。

```powershell
python scripts/prepare_generation_evaluation.py
python scripts/generate.py --input data/processed/week1_v3/generation_evaluation_samples.jsonl --sample-count 100 --output reports/generation/baseline/evaluation_outputs.json --offline
...
试跑结果写入 `reports/generation/baseline/smoke_test.json`，正式100条输出写入 `reports/generation/baseline/evaluation_outputs.json`。报告保存真实模型输入、原始输出、解析结果、运行时间和 PyTorch 显存。`reference_title_not_given_to_model` 只用于结果对照，没有送入提示词。需要只测一条时省略 `--sample-count 5`。
